...
//Change the state based on if someone needs to move in or out after the person in the correct row finishes storing luggage
...
void updateRow(StackAr<Row> &rows, Row &row, Queue<Seat> &passengers, int rowNum) {
    switch(row.state) {
        case NONE: {
...
int simulateBoarding(Queue<Seat> passengers) {
    ...
    //Add the first passenger in first
...
Queue<Seat> readPassengers(ifstream &input) {
...
int main(int argc, char **argv) {
    ...
    Queue<Seat> passengers = readPassengers(input);
    cout << "Back to front: " << simulateBoarding(passengers) << endl;
    passengers = readPassengers(input);
