...
int **table;
FibHeapNode ** heap1Locs;
FibHeapNode ** heap2Locs;
Road **adjList;
Road empty;
int tableSize;
int totalRoads;
...
Tripper::Tripper(Road *roads, int numRoads, int size) {
  tableSize = size * size;
  totalRoads = numRoads;
  heap1Locs = new FibHeapNode*[tableSize];
  heap2Locs = new FibHeapNode*[tableSize];
...
void Tripper::findCity(int city1, int city2, int &dist, int &connection) {
  FibHeap heap1; //prob has memory leak with left over nodes, maybe move to global and reuse since we need to delete what's in it to avoid leak
  FibHeap heap2; //prob has memory leak with left over nodes, maybe move to global and reuse since we need to delete what's in it to avoid leak
  initializeTable();
  
  table[city1][1] = 0;
  VertexNode *start = new VertexNode(city1, 0);
  heap1Locs[city1] = start;
  heap1.Insert(start);
  
  table[city2][1] = 0;
  VertexNode *start2 = new VertexNode(city2, 0);
  heap2Locs[city1] = start2;
  heap2.Insert(start2);
...
bool Tripper::dijkstraStep(FibHeap &heap, int colOffset, FibHeapNode * heapLocs[]) {
  //Take min off top
  //cout << "rem" << endl;
  FibHeapNode *min = heap.ExtractMin();
  int minCity = ((VertexNode*) min)->city;
  int minDist = ((VertexNode*) min)->dist;
  delete min;
  heapLocs[minCity] = 0;

  //Terminate if to be scanned was already scanned in other
  table[minCity][2+colOffset] = 1;
  if(table[minCity][2] == table[minCity][5]) {
    return true;
  }
  
  //Loop through all outgoing roads to adjacent cities
  int i = 0;
  while(i != 8 && adjList[minCity][i].distance != -1) {
    Road currRoad = adjList[minCity][i];
    i++;
    ...
    int newDv = minDist + currRoad.distance;
    if(newDv < table[currRoad.city2][1+colOffset]) { //found better path
      table[currRoad.city2][0+colOffset] = minCity;
      table[currRoad.city2][1+colOffset] = newDv;
      
      if(!heapLocs[currRoad.city2]) {
        cout << "ins" << endl;
        VertexNode *city2 = new VertexNode(currRoad.city2, newDv);
        heapLocs[currRoad.city2] = city2;
        heap.Insert(city2);
      } else {
        //cout << "dec" << endl;
        VertexNode city2(currRoad.city2, newDv);
