...
void createRoads(City **cities, int size)
{
  ...
  for(int i = 0; i < size - 1; i++)
  {
   for(int j = 0; j < size; j++)
    {
      cities[i][j].roads[E] = cities[i + 1][j].roads[W] = rand() % 15 + 3;
      cities[j][i].roads[S] = cities[j][i + 1].roads[N] = rand() % 15 + 5;
    ...
    for(int NS = 0; NS < size; NS++)
    {
      ...
      if(WE < size - 1 && NS > 0)
      {
        distance = (int) sqrt(cities[WE][NS].roads[N] * cities[WE + 1][NS].roads[N]
          + cities[WE][NS].roads[S] * cities[WE][NS - 1].roads[S]) + (rand() % 4) - 2;
